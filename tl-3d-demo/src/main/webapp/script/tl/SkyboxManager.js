...
import { CAMERA_MOVE_DURATION, FLOOR_PADDING, WHITE } from "./Constants.js";
...
export class SkyboxManager {
  ...
  toggleSkybox(visible) {
    ...
    [this.environmentBackground, ...(this.factoryFloors || [])].forEach(
      (obj) => {
  ...
  async loadCubeTexture() {
    ...
    const lrWallUrl =
      "https://as2.ftcdn.net/v2/jpg/03/24/83/79/1000_F_324837965_WzkElQWESPpFBltfwddlJaQIYH8X9D5V.jpg"; // Left/Right walls
    const fbWallUrl =
      "https://as2.ftcdn.net/v2/jpg/01/98/99/89/1000_F_198998931_VZjbdpDglkKPjbqaXa3bnH8fB6axUTcp.jpg"; // Front/Back walls
    const floorUrl =
      "https://as1.ftcdn.net/v2/jpg/01/79/71/02/1000_F_179710260_hVhHw5vQNsXyb9qvIkRbIHBzWUuVcmF7.jpg"; // Floor/Ceiling
  ...
  createEnvironmentCube(cubeTexture = null) {
    ...
    if (cubeTexture) {
      ...
      const geometry = new BoxGeometry(floorSizes.x, y * 2, floorSizes.z).scale(
        -1,
        1,
        1,
      );
      this.environmentBackground = new Mesh(
        geometry,
        cubeTexture.images.map(
          (img) =>
            new MeshBasicMaterial({
              map: img ? new CanvasTexture(img) : null,
              side: FrontSide,
            }),
        ),
      );
  ...
  createFactoryFloors(cubeTexture) {
    ...
    for (const level of floorsToCreate) {
      ...
      const textTexture = this.createFloorTextTexture(
        `FLOOR ${level}`,
        floorImage,
      );
  ...
  zoomToSkyboxDistance() {
    ...
    gsap.to(this.control.controls.target, {
      x: center.x,
      y: center.y,
      z: center.z,
      duration: CAMERA_MOVE_DURATION,
